  ...
  return (
    <div className="space-y-7">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <article className="card p-5 sm:p-7"><div className="flex items-start justify-between gap-4 border-b border-[#e4ece6] pb-5"><div><div className="flex items-center gap-2"><SlidersHorizontal className="text-spruce" size={19} /><h2 className="text-xl font-semibold tracking-[-.04em] text-ink">Acceptance profile</h2></div><p className="mt-2 text-sm leading-6 text-[#687e75]">Matching evaluates these explicit rules before it recommends a commercial action.</p></div><DemoBadge>buyer controlled</DemoBadge></div>
          ...
          <div className="mt-6 flex flex-col gap-3 border-t border-[#e4ece6] pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-[#6e837a]">Template updated: {new Date(spec.updated_at).toLocaleDateString('en-IN')} · Demo template version</p><button className="btn-primary" disabled={saving} onClick={() => void save()}>{saving ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}{saving ? 'Saving…' : 'Save acceptance template'}</button></div>
        </article>
