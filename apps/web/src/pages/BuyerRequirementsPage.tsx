...
import { StatusBadge, Disclosure, EmptyPanel, ErrorPanel, LoadingPanel, PageHeader, QualityPill, ScoreRing } from '../components/ui'
...
export function BuyerRequirementsPage({ role }: { role: Role }) {
  ...
  return (
    <div className="space-y-7">
      ...
      {selectedRequirementId && <section id="supply-analysis" className="scroll-mt-6 space-y-4 rounded-3xl border border-[#d7e6dc] bg-[#f8fcf9] p-5 sm:p-7"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">Supply analysis</p><h2 className="mt-1 section-title">Compatible waste listings</h2><p className="mt-1 text-sm text-[#6c8179]">Ranked using the same decision rules; source quality flags remain visible.</p></div>{matches.loading ? <span className="text-sm text-[#6b8179]">Analyzing…</span> : matches.data ? <span className="badge-safe">{matches.data.matches.length} passed compatibility gates</span> : null}</div>{matches.loading ? <LoadingPanel label="Comparing active listings…" /> : matches.error ? <ErrorPanel error={matches.error} onRetry={() => void matches.reload()} /> : matches.data?.matches.length ? <div className="grid gap-4 xl:grid-cols-3">{matches.data.matches.map((match) => { const listing = match.waste_listing; return <article key={match.id} className="card card-interactive p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-semibold tracking-[-0.03em] text-ink">{listing?.company || 'Waste generator'}</p><p className="mt-1 text-xs text-[#70857e]">{listing?.city} · {listing?.material}</p></div><ScoreRing score={match.total_score} size={72} /></div><div className="mt-4 flex items-center gap-2"><QualityPill verified={listing?.quality_verified || false} grade={titleCase(listing?.quality_grade)} /></div><div className="mt-4 space-y-2 text-xs text-[#5e756c]"><p><strong className="text-ink">Available:</strong> {formatKg(listing?.normalized_kg_per_week || 0)}/week</p><p><strong className="text-ink">Distance:</strong> {match.distance_km.toFixed(1)} km</p><p><strong className="text-ink">Potential use:</strong> {match.potential_use}</p></div><Link className="btn-primary mt-5 w-full" to={`/matches/${match.id}`}><PackageSearch size={16} />View explanation</Link></article> })}</div> : <EmptyPanel title="No compatible supply found" detail="Try extending the maximum distance, widening the quantity range, or choose another supported material." />}</section>}
    </div>
  )
}
