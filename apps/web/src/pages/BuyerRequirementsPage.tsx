    ...
    <div className="space-y-7">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_312px] xl:grid-cols-[minmax(0,1fr)_390px]">
        <div className="card p-5 sm:p-7">
          <div className="flex items-start justify-between gap-3 border-b border-[#e4ece6] pb-5"><div><div className="flex items-center gap-2"><Factory size={19} className="text-spruce" /><h2 className="text-lg font-semibold tracking-[-0.03em] text-ink">{editingId ? 'Edit buyer requirement' : 'New buyer requirement'}</h2></div><p className="mt-1 text-sm text-[#6a8078]">One material per requirement keeps matching clear and explainable.</p></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#e7f4ec] text-spruce">{editingId ? <Edit2 size={18} /> : <Plus size={18} />}</span></div>
          <div className="mt-5 grid gap-6 sm:grid-cols-2">
            <label><span className="field-label">Required material</span><select className="field-input w-full" disabled={!!editingId} value={form.material_id} onChange={(event) => update('material_id', event.target.value)}>{materials.data.map((material) => <option key={material.id} value={material.id}>{material.canonical_name}</option>)}</select></label>
          ...
          <div className="mt-6 flex flex-col gap-3 border-t border-[#e4ece6] pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-[#71867e]"><Ruler className="mr-1 inline text-spruce" size={14} />Distance uses demo Haversine calculations between sample city coordinates.</p>
            <div className="flex gap-3 w-full sm:w-auto">
              {editingId && <button className="btn-secondary w-full sm:w-auto" onClick={() => { setEditingId(null); setForm({ ...form, minimum_quantity_kg_week: '2000', maximum_quantity_kg_week: '5000' }) }}>Cancel</button>}
              <button className="btn-primary w-full sm:w-auto" disabled={saving} onClick={() => void submitRequirement()}>{saving ? <Loader2 className="animate-spin" size={17} /> : <Target size={17} />}{saving ? 'Saving…' : (editingId ? 'Save changes' : 'Publish & find supply')}<ArrowRight size={16} /></button>
            </div>
          </div>
        </div>
      ...
      <section className="space-y-4">
        ...
        {requirements.data.length === 0 ? <EmptyPanel title="No buyer requirement yet" detail="Publish a controlled material requirement above to analyze compatible supply." /> : <div className="grid gap-4 lg:grid-cols-2">{requirements.data.map((requirement) => <article key={requirement.id} className={`card card-interactive p-5 ${selectedRequirementId === requirement.id ? 'border-spruce ring-4 ring-mint/30' : ''}`}><div className="flex justify-between gap-4"><div><p className="text-lg font-semibold tracking-[-0.03em] text-ink">{requirement.material}</p><p className="mt-1 text-xs text-[#6f857c]">{requirement.company} · {requirement.city}</p></div><div className="flex items-center gap-2"><span className="rounded-xl bg-[#e7f4ec] px-3 py-2 text-xs font-bold text-spruce">Active</span><button className="grid h-9 w-9 place-items-center rounded-xl bg-[#f5f8f5] text-[#81968d] hover:bg-[#e7f4ec] hover:text-spruce transition-colors" onClick={() => handleEdit(requirement)} aria-label="Edit requirement"><Edit2 size={16} /></button><button className="grid h-9 w-9 place-items-center rounded-xl bg-[#f5f8f5] text-[#81968d] hover:bg-[#ffedeb] hover:text-[#d33d3d] transition-colors" onClick={() => setConfirmDeleteId(requirement.id)} aria-label="Archive requirement"><Trash2 size={16} /></button></div></div><div className="mt-5 grid grid-cols-2 gap-3 rounded-2xl bg-[#f5f8f5] p-3 text-xs"><div><span className="block text-[#789087]">Quantity</span><strong className="mt-1 block text-ink">{formatKg(requirement.minimum_quantity_kg_week)}–{formatKg(requirement.maximum_quantity_kg_week)}</strong></div><div><span className="block text-[#789087]">Radius</span><strong className="mt-1 block text-ink">{requirement.maximum_distance_km} km</strong></div><div><span className="block text-[#789087]">Target</span><strong className="mt-1 block text-ink">{formatCurrency(requirement.target_price_per_kg)}/kg</strong></div><div><span className="block text-[#789087]">Minimum quality</span><strong className="mt-1 block text-ink">{titleCase(requirement.minimum_quality_grade)}</strong></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2"><button className={selectedRequirementId === requirement.id ? "btn-primary w-full" : "btn-secondary w-full"} onClick={() => handleSelectRequirement(requirement.id)}><PackageSearch size={16} />{selectedRequirementId === requirement.id ? 'Viewing Suppliers' : 'Find Suppliers'}</button><Link className="btn-primary w-full" to={`/buyer-requirements/${requirement.id}/acceptance-spec`}><Ruler size={16} />Acceptance rules</Link></div></article>)}</div>}
      </section>

      {selectedRequirementId && <section id="supply-analysis" className="scroll-mt-6 space-y-4 rounded-3xl border border-[#d7e6dc] bg-[#f8fcf9] p-5 sm:p-7"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">Supply analysis</p><h2 className="mt-1 section-title">Compatible waste listings</h2><p className="mt-1 text-sm text-[#6c8179]">Ranked using the same decision rules; source quality flags remain visible.</p></div>{matches.loading ? <span className="text-sm text-[#6b8179]">Analyzing…</span> : matches.data ? <span className="badge-safe">{matches.data.matches.length} passed compatibility gates</span> : null}</div>{matches.loading ? <PageSkeleton /> : matches.error ? <ErrorPanel error={matches.error} onRetry={() => void matches.reload()} /> : matches.data?.matches.length ? <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">{matches.data.matches.map((match) => { const listing = match.waste_listing; return <article key={match.id} className="card card-interactive p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-semibold tracking-[-0.03em] text-ink">{listing?.company || 'Waste generator'}</p><p className="mt-1 text-xs text-[#70857e]">{listing?.city} · {listing?.material}</p></div><ScoreRing score={match.total_score} size={72} /></div><div className="mt-4 flex items-center gap-2"><QualityPill verified={listing?.quality_verified || false} grade={titleCase(listing?.quality_grade)} /></div><div className="mt-4 space-y-2 text-xs text-[#5e756c]"><p><strong className="text-ink">Available:</strong> {formatKg(listing?.normalized_kg_per_week || 0)}/week</p><p><strong className="text-ink">Distance:</strong> {match.distance_km.toFixed(1)} km</p><p><strong className="text-ink">Potential use:</strong> {match.potential_use}</p></div><Link className="btn-primary mt-5 w-full" to={`/matches/${match.id}`}><PackageSearch size={16} />View explanation</Link></article> })}</div> : <EmptyPanel title="No compatible supply found" detail="Try extending the maximum distance, widening the quantity range, or choose another supported material." />}</section>}
    </div>
