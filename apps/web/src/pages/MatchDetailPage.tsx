...
export function MatchDetailPage({ role }: { role: Role }) {
  ...
  const [workflowConfirm, setWorkflowConfirm] = useState<{ type: 'sample_accept' | 'offer_accept' | 'shipment' | 'receipt' } | null>(null)
  ...
  const contact = async () => {
    ...
    setActionError(null)
    try {
      const response = await post<{ message: string; email_sent: boolean }>(`/api/matches/${matchId}/contact`, { note: contactNote.trim() || 'Interested in this match — please get in touch.' })
      setMessage(response.data.message)
      setShowContactModal(false)
      setContactNote('')
      void detail.reload()
    } catch (cause) {
      setActionError(cause instanceof Error ? cause.message : 'Could not send contact request.')
    } finally {
      setContacting(false)
    }
  }

  if (detail.loading) return <PageSkeleton />
  ...
  return (
    <div className="space-y-7 animate-fade-in-up">
      ...
      <ConfirmDialog 
        isOpen={workflowConfirm?.type === 'offer_accept'}
        title="Accept offer"
        description="Are you sure you want to accept this commercial offer?"
        onCancel={() => setWorkflowConfirm(null)}
        onConfirm={() => { void runWorkflow('offer_accept'); setWorkflowConfirm(null); }}
      />

      <ConfirmDialog 
        isOpen={workflowConfirm?.type === 'receipt'}
        title="Record demo receipt"
        description="Are you sure you want to mark this shipment as received?"
        onCancel={() => setWorkflowConfirm(null)}
        onConfirm={() => { void runWorkflow('receipt'); setWorkflowConfirm(null); }}
      />

      <ConfirmDialog 
        isOpen={workflowConfirm?.type === 'shipment'}
        title="Plan dispatch / pickup"
        description="Please confirm the pickup date for this dispatch."
        onCancel={() => setWorkflowConfirm(null)}
        onConfirm={() => { void runWorkflow('shipment'); setWorkflowConfirm(null); }}
      >
        <label className="mb-1.5 block text-sm font-medium text-ink">Pickup Date</label>
        <input 
          type="date" 
      ...
      <section className="grid gap-5 lg:grid-cols-2 xl:grid-cols-[.86fr_1.14fr]">
      ...
      <section className="card overflow-hidden"><div className="flex flex-col gap-4 border-b border-[#dfe9e2] bg-gradient-to-r from-[#f2faf4] to-white p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7"><div><p className="eyebrow">Operational workflow</p><h2 className="mt-2 text-xl font-semibold tracking-[-.04em] text-ink">Sample → offer → pickup → receipt</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#647a71]">These controls create clearly labelled demo records. In production, they become evidence-backed operational handoffs rather than automatic deal closure.</p></div><span className="badge-neutral">active workflow</span></div><div className="grid gap-5 p-5 sm:p-7 lg:grid-cols-2 xl:grid-cols-[.68fr_1.32fr]"><div className="space-y-3">
        {role === 'buyer' && (
          <>
            <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('sample')}>{workflowBusy === 'sample' ? <Loader2 className="animate-spin" size={16} /> : <FlaskConical size={16} />}Request sample / inspection</button>
            <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('offer')}>{workflowBusy === 'offer' ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />}Make commercial offer</button>
            {latestShipment && latestShipment.status !== 'received' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => setWorkflowConfirm({ type: 'receipt' })}>{workflowBusy === 'receipt' ? <Loader2 className="animate-spin" size={16} /> : <PackageCheck size={16} />}Record demo receipt</button>}
          </>
        )}
        {role === 'generator' && (
          <>
            {latestSample && latestSample.status !== 'accepted' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('sample_accept')}>{workflowBusy === 'sample_accept' ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}Accept sample request</button>}
            {latestOffer && latestOffer.status !== 'accepted' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => setWorkflowConfirm({ type: 'offer_accept' })}>{workflowBusy === 'offer_accept' ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}Accept buyer offer</button>}
            <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => setWorkflowConfirm({ type: 'shipment' })}>{workflowBusy === 'shipment' ? <Loader2 className="animate-spin" size={16} /> : <Truck size={16} />}Plan dispatch / pickup</button>
          </>
        )}
        {operationalBlocked && <p className="rounded-xl border border-[#f1d1c5] bg-[#fff5f0] p-3 text-xs leading-5 text-[#99513c]">Resolve the explicit blocked gate before advancing the workflow.</p>}</div><div className="space-y-3">{data.timeline.length ? data.timeline.map((event) => <div key={event.id} className="flex gap-3 rounded-2xl border border-[#deebe2] bg-[#fbfdfb] p-4"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[#e9f6ee] text-spruce">{event.type === 'sample_request' ? <FlaskConical size={15} /> : event.type === 'offer' ? <Send size={15} /> : event.type === 'shipment' ? <Truck size={15} /> : <Contact size={15} />}</span><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold text-ink">{event.title}</p><span className="badge-neutral">{titleCase(event.status)}</span><span className="ml-auto text-xs text-[#a0b1a9]">{new Date(event.created_at).toLocaleDateString()}</span></div><p className="mt-1 text-xs leading-5 text-[#627970]">{event.detail}</p></div></div>) : <p className="rounded-2xl bg-[#f7faf8] p-5 text-sm text-[#6b8179]">No sample, offer, pickup, or receipt events have been recorded yet.</p>}</div></div></section>
      ...
      <section className="space-y-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">Visualize the proposed route</p><h2 className="mt-1 section-title">Generator → buyer</h2></div><span className="badge-warn"><Truck className="mr-1" size={13} />{data.map_route.label}</span></div>{map.loading ? <PageSkeleton /> : map.error || !map.data ? <ErrorPanel error={map.error || 'Route data unavailable.'} onRetry={() => void map.reload()} /> : <NetworkMap points={map.data.points} route={map.data.selected_route} />}</section>
