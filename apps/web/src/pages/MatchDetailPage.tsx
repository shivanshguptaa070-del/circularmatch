...
export function MatchDetailPage({ role }: { role: Role }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="card overflow-hidden"><div className="flex flex-col gap-4 border-b border-[#dfe9e2] bg-gradient-to-r from-[#f2faf4] to-white p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7"><div><p className="eyebrow">Operational workflow</p><h2 className="mt-2 text-xl font-semibold tracking-[-.04em] text-ink">Sample → offer → pickup → receipt</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#647a71]">These controls create clearly labelled demo records. In production, they become evidence-backed operational handoffs rather than automatic deal closure.</p></div><span className="badge-neutral">active workflow</span></div><div className="grid gap-5 p-5 sm:p-7 xl:grid-cols-[.68fr_1.32fr]"><div className="space-y-3">
        {role === 'buyer' && (
          <>
            ...
            {latestShipment && latestShipment.status !== 'received' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('receipt')}>{workflowBusy === 'receipt' ? <Loader2 className="animate-spin" size={16} /> : <PackageCheck size={16} />}Record demo receipt</button>}
          </>
        )}
        {role === 'generator' && (
          <>
            ...
            {latestOffer && latestOffer.status !== 'accepted' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('offer_accept')}>{workflowBusy === 'offer_accept' ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}Accept buyer offer</button>}
            <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('shipment')}>{workflowBusy === 'shipment' ? <Loader2 className="animate-spin" size={16} /> : <Truck size={16} />}Plan dispatch / pickup</button>
          </>
        )}
