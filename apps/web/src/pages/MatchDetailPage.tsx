  ...
  return (
    <div className="space-y-7">
      <PageHeader
        ...
        actions={<><Link className="btn-secondary" to={`/listings/${listing.id}/passport`}><ClipboardCheck size={16} />Material Passport</Link>{role === 'buyer' ? <Link className="btn-secondary" to="/supply"><ArrowRight className="rotate-180" size={16} />Back to supply</Link> : <Link className="btn-secondary" to={`/listings/${listing.id}/matches`}><ArrowRight className="rotate-180" size={16} />Back to buyers</Link>}</>}
      />
      ...
      <section className="card overflow-hidden"><div className="flex flex-col gap-4 border-b border-[#dfe9e2] bg-gradient-to-r from-[#f2faf4] to-white p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7"><div><p className="eyebrow">Operational workflow</p><h2 className="mt-2 text-xl font-semibold tracking-[-.04em] text-ink">Sample → offer → pickup → receipt</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#647a71]">These controls create clearly labelled demo records. In production, they become evidence-backed operational handoffs rather than automatic deal closure.</p></div><span className="badge-neutral">active workflow</span></div><div className="grid gap-5 p-5 sm:p-7 xl:grid-cols-[.68fr_1.32fr]"><div className="space-y-3">{(role === 'buyer' || role === 'admin') && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('sample')}>{workflowBusy === 'sample' ? <Loader2 className="animate-spin" size={16} /> : <FlaskConical size={16} />}Request sample inspection</button>}{role === 'generator' && !latestSample && <p className="rounded-xl bg-[#f7faf8] p-3 text-xs leading-5 text-[#6c8178]">Awaiting buyer sample request.</p>}{(role === 'generator' || role === 'admin') && latestSample && latestSample.status !== 'accepted' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('sample_accept')}>{workflowBusy === 'sample_accept' ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}Approve & dispatch sample</button>}{(role === 'buyer' || role === 'admin') && latestSample && latestSample.status !== 'accepted' && <p className="rounded-xl bg-[#fff8e8] p-3 text-xs leading-5 text-[#866a25]">Awaiting seller to approve sample.</p>}{(role === 'buyer' || role === 'admin') && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('offer')}>{workflowBusy === 'offer' ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />}Send purchase offer</button>}{role === 'generator' && !latestOffer && <p className="rounded-xl bg-[#f7faf8] p-3 text-xs leading-5 text-[#6c8178]">Awaiting buyer commercial offer.</p>}{(role === 'generator' || role === 'admin') && latestOffer && latestOffer.status !== 'accepted' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('offer_accept')}>{workflowBusy === 'offer_accept' ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}Accept purchase offer</button>}{(role === 'buyer' || role === 'admin') && latestOffer && latestOffer.status !== 'accepted' && <p className="rounded-xl bg-[#fff8e8] p-3 text-xs leading-5 text-[#866a25]">Awaiting seller to accept offer.</p>}{(role === 'generator' || role === 'admin') && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('shipment')}>{workflowBusy === 'shipment' ? <Loader2 className="animate-spin" size={16} /> : <Truck size={16} />}Schedule & dispatch pickup</button>}{(role === 'buyer' || role === 'admin') && latestShipment && latestShipment.status !== 'received' && <button className="btn-secondary w-full" disabled={operationalBlocked || workflowBusy !== null} onClick={() => void runWorkflow('receipt')}>{workflowBusy === 'receipt' ? <Loader2 className="animate-spin" size={16} /> : <PackageCheck size={16} />}Confirm delivery & weighbridge receipt</button>}{role === 'generator' && latestShipment && latestShipment.status !== 'received' && <p className="rounded-xl bg-[#f7faf8] p-3 text-xs leading-5 text-[#6c8178]">Awaiting buyer receipt confirmation.</p>}{operationalBlocked && <p className="rounded-xl border border-[#f1d1c5] bg-[#fff5f0] p-3 text-xs leading-5 text-[#99513c]">Resolve the explicit blocked gate before creating a sample, offer, or pickup record.</p>}</div><div className="space-y-3">{data.timeline.length ? data.timeline.map((event) => <div key={event.id} className="flex gap-3 rounded-2xl border border-[#deebe2] bg-[#fbfdfb] p-4"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[#e9f6ee] text-spruce">{event.type === 'sample_request' ? <FlaskConical size={15} /> : event.type === 'offer' ? <Send size={15} /> : event.type === 'shipment' ? <Truck size={15} /> : <Contact size={15} />}</span><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold text-ink">{event.title}</p><span className="badge-neutral">{titleCase(event.status)}</span></div><p className="mt-1 text-xs leading-5 text-[#627970]">{event.detail}</p></div></div>) : <p className="rounded-2xl bg-[#f7faf8] p-5 text-sm text-[#6b8179]">No sample, offer, pickup, or receipt events have been recorded yet.</p>}</div></div></section>
