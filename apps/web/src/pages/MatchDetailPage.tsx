...
import { DemoBadge, Disclosure, ErrorPanel, LoadingPanel, PageHeader, QualityPill, ScoreBar, ScoreRing } from '../components/ui'
...
export function MatchDetailPage({ role }: { role: Role }) {
  ...
  const runWorkflow = async (type: 'sample' | 'sample_accept' | 'offer' | 'offer_accept' | 'shipment' | 'receipt') => {
    ...
    try {
      ...
      if (type === 'sample') {
        response = await post<{ message: string }>(`/api/matches/${matchId}/sample-requests`, { requested_quantity_kg: 25, note: 'Illustrative demo sample request created from the match workspace.' })
        ...
        response = await post<{ message: string }>(`/api/matches/${matchId}/offers`, {
          ...
          note: 'Illustrative demo offer only — not a binding commercial agreement.',
        })
        ...
        response = await patch<{ message: string }>(`/api/offers/${offer.record.id}`, { status: 'accepted', note: 'Illustrative demo offer acceptance; commercial terms still require real agreement.' })
        ...
        response = await patch<{ message: string }>(`/api/shipments/${shipment.record.id}`, { status: 'received', dispatched_weight_kg: receiptWeight, received_weight_kg: receiptWeight, receipt_note: 'Illustrative demo receipt; replace with weighbridge/receipt evidence in production.' })
      }
      setMessage(response.data.message)
      await detail.reload()
    } catch (cause) {
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="grid gap-5 xl:grid-cols-[.86fr_1.14fr]">
        ...
        <article className="card p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Transparent score</p><h2 className="mt-2 text-xl font-semibold tracking-[-0.035em] text-ink">Decision breakdown</h2><p className="mt-1 text-sm leading-6 text-[#6d827a]">Components remain deterministic and versioned. They do not override a blocked or missing-evidence gate.</p></div><DemoBadge>v{match.scoring_config_id.split('-').pop()}</DemoBadge></div><div className="mt-7 space-y-5">{explanation.score_breakdown.map((item) => <ScoreBar key={item.key} label={item.label} score={item.score} weight={item.weight} />)}</div><div className="mt-7 rounded-2xl border border-[#dce9e0] bg-[#f5faf6] p-4"><p className="text-xs font-semibold text-ink">{explanation.decision_rule_label}</p><p className="mt-1 text-xs leading-5 text-[#678078]">A score is a comparison aid. It does not mean material acceptance, quality certification, compliance clearance, or transaction success.</p></div></article>
      </section>
