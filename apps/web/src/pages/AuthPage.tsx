        ...
        >
          ...
          {step === 'reset' && (
            <div className="mb-6 text-center">
              ...
              <p className="mt-1 text-sm text-[#7ab09a]">Enter your email to receive a reset link</p>
            </div>
          )}
          ...
          <form onSubmit={step === 'reset' ? handleResetPassword : handleEmailAuth} className="space-y-4">
            ...
            {step !== 'reset' && (
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <label className="block text-xs font-semibold text-[#9ec9b5]" htmlFor="auth-password">Password</label>
                  {step === 'signin' && (
                    <button type="button" onClick={() => { setStep('reset'); setError(null); setSuccessMsg(null); }} className="text-xs font-medium text-mint hover:underline">
                      Forgot password?
                    </button>
                  )}
                </div>
            ...
            {successMsg && (
              <div className="flex flex-col gap-3 rounded-xl border border-mint/30 bg-mint/10 p-3 text-sm text-mint">
                ...
                {step === 'signup' && successMsg.includes('verify your account') && (
                  <button 
                    ...
                    className="self-start text-xs font-semibold underline hover:text-white"
                  >
            ...
            >
              ...
              {loading ? (step === 'signin' ? 'Signing in…' : step === 'reset' ? 'Sending...' : 'Creating account…') : (
                <>{step === 'signin' ? 'Sign in' : step === 'reset' ? 'Send reset link' : 'Create account'} <ArrowRight size={16} /></>
              )}
            </button>
            ...
            {step === 'reset' && (
              <div className="text-center">
                <button
                  ...
                  className="text-sm font-medium text-[#7ab09a] hover:text-white"
                >
                  Back to Sign In
                </button>
              </div>
