...
def create_presentation():
    ...
    def add_slide_base(slide, title_chip, main_title, subtitle=None):
        ...
        ftf.word_wrap = True
        fp = ftf.paragraphs[0]
        fp.text = "♻ CircularMatch  |  HACKDAY 1.0  |  Solo Levelling — Shivansh Gupta"
        fp.font.size = Pt(10)
    ...
    # Left Column: Pain Points Card
    card1 = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.8), Inches(6.0), Inches(4.3))
    card1.fill.solid()
    ...
    p.text = "Industrial buyers reject 30–60% of material upon physical arrival — wrong grade, mismatched technical specs, or undisclosed contamination causing entire truckloads to turn back."
    p.font.size = Pt(11)
    ...
    p = ctf1.add_paragraph()
    p.text = "⚖️ Informal Brokers = Legal Liability"
    p.font.size = Pt(14)
    p.font.bold = True
    p.font.color.rgb = DANGER_RED

    p = ctf1.add_paragraph()
    p.text = "Brokers move material fast but produce no compliant paper trail — creating direct regulatory liability for manufacturers filing with SEBI."
    p.font.size = Pt(11)
    p.font.color.rgb = MUTED
    ...
    img_path1 = os.path.join(screenshots_dir, 'slide1_landing.png')
    if os.path.exists(img_path1):
        s1.shapes.add_picture(img_path1, Inches(7.1), Inches(1.8), width=Inches(5.4))
    ...
    for i, (f_title, f_desc) in enumerate(features):
        x = Inches(0.8 + (i % 2) * 3.1)
        y = Inches(1.8 + (i // 2) * 1.5)
        card = s2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, x, y, Inches(2.95), Inches(1.35))
        card.fill.solid()
    ...
    # Core Flow Banner
    flow_box = s2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(5.0), Inches(6.05), Inches(1.8))
    flow_box.fill.solid()
    ...
    # Right Column: Screenshot (Material Passport)
    img_path2 = os.path.join(screenshots_dir, 'slide2_passport.png')
    if os.path.exists(img_path2):
        s2.shapes.add_picture(img_path2, Inches(7.1), Inches(1.8), width=Inches(5.4))
    ...
    # Supplier Box
    sup_card = s3.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.8), Inches(5.7), Inches(4.3))
    sup_card.fill.solid()
    sup_card.fill.fore_color.rgb = CARD_BG
    sup_card.line.color.rgb = GREEN_PRIMARY
    s_tf = sup_card.text_frame
    s_tf.word_wrap = True
    s_tf.margin_left = s_tf.margin_top = s_tf.margin_right = s_tf.margin_bottom = Inches(0.2)
    ...
    sp.text = "Profile: BSE/NSE-listed manufacturing enterprise in Noida or Greater Noida generating recurring industrial waste (cotton cutting scraps, PET thermoforming, cardboard) with mandatory BRSR Core filing obligations."
    sp.font.size = Pt(11)
    sp.font.color.rgb = WHITE
    sp.space_before = Pt(8)

    sp = s_tf.add_paragraph()
    sp.text = "Their Pain: BRSR auditors now reject informal broker receipts. They need digital chain-of-custody proof for statutory audits. Currently, they have zero digital documentation."
    sp.font.size = Pt(11)
    sp.font.color.rgb = DANGER_RED
    sp.space_before = Pt(10)

    # Inset screenshot for Supplier
    img_sup = os.path.join(screenshots_dir, 'slide3_seller_dashboard.png')
    if os.path.exists(img_sup):
        s3.shapes.add_picture(img_sup, Inches(1.0), Inches(3.9), width=Inches(5.3))

    # Buyer Box
    buy_card = s3.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.8), Inches(1.8), Inches(5.7), Inches(4.3))
    buy_card.fill.solid()
    buy_card.fill.fore_color.rgb = CARD_BG
    buy_card.line.color.rgb = TEAL_ACCENT
    b_tf = buy_card.text_frame
    b_tf.word_wrap = True
    b_tf.margin_left = b_tf.margin_top = b_tf.margin_right = b_tf.margin_bottom = Inches(0.2)
    ...
    bp.text = "Profile: Sourcing manager at a registered recycler or corporate brand owner building EPR compliance supply chains — needing pre-qualified, specification-matched, documented supply."
    bp.font.size = Pt(11)
    bp.font.color.rgb = WHITE
    bp.space_before = Pt(8)

    bp = b_tf.add_paragraph()
    bp.text = "Their Pain: 30–60% gate rejection rate on new suppliers. Every rejected shipment equals 100% freight and logistics cost with zero usable material received."
    bp.font.size = Pt(11)
    bp.font.color.rgb = DANGER_RED
    bp.space_before = Pt(10)

    # Inset screenshot for Buyer
    img_buy = os.path.join(screenshots_dir, 'slide3_buyer_dashboard.png')
    if os.path.exists(img_buy):
        s3.shapes.add_picture(img_buy, Inches(7.0), Inches(3.9), width=Inches(5.3))
    ...
    # AI Features Callout under Table
    ...
    # Right Column: Screenshot (Match Breakdown)
    img_path4 = os.path.join(screenshots_dir, 'slide4_match_detail.png')
    ...
    # Right: Screenshot (Impact & Economics)
    img_path5 = os.path.join(screenshots_dir, 'slide5_impact.png')
    ...
    # Right: Screenshot (Admin Config)
    img_path6 = os.path.join(screenshots_dir, 'slide6_admin.png')
    ...
    # Right: Screenshot (Map Geospatial Routing)
    img_path7 = os.path.join(screenshots_dir, 'slide7_map.png')
